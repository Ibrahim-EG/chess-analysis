...
object StockfishNative {
    private var engine: fr.axl_lvy.stockfish_multiplatform.StockfishEngine? = null
    suspend fun init() { if (engine == null) engine = getStockfish() }
    
    suspend fun evaluate(fen: String, depth: Int = 12): Pair<String, Int> {
        init()
        val eng = engine!!
        eng.setPosition(fen)
        val result = eng.search(depth)
        val bestMove = result.bestMove ?: ""
        var cp = 0
        try {
            val methods = result::class.java.methods
            val evalMethod = methods.firstOrNull { it.name.contains("eval", true) || it.name.contains("score", true) || it.name.contains("cp", true) }
            if (evalMethod != null) {
                val res = evalMethod.invoke(result)
                if (res is Number) cp = res.toInt()
                else if (res != null) {
                    val cpMethod = res::class.java.methods.firstOrNull { it.name.contains("cp", true) || it.name.contains("value", true) }
                    if (cpMethod != null) { val cpVal = cpMethod.invoke(res); if (cpVal is Number) cp = cpVal.toInt() }
                }
            }
        } catch (_: Exception) {}
        return Pair(bestMove, cp)
    }
}
...
data class UiState(
    ...
    val topLineCp: Int = 0, val searching: Boolean = false, val status: String = "Warming up the Lamp...",
...
class ChessVm(app: Application) : AndroidViewModel(app) {
    ...
    init {
        viewModelScope.launch {
            StockfishNative.init()
            _ui.update { it.copy(status = "Stockfish 18 Ready") }
            analyzeCurrent()
        }
    }
    ...
    private fun doMove(from: Square, to: Square) {
        ...
        board.doMove(mv); val uci = uciOf(mv); val newHistory = _ui.value.history + uci
        
        _ui.update { st -> st.copy(fen = board.fen, pieces = snapshotPieces(), selected = null, legalTargets = emptySet(),
            lastMove = from to to, history = newHistory, gameOver = board.isMated || board.isDraw, lastBadge = null) }
        
        viewModelScope.launch {
            val (bestBefore, cpBefore) = StockfishNative.evaluate(fenBefore, 12)
            val (bestAfter, cpAfter) = StockfishNative.evaluate(board.fen, 12)
            val loss = if (stmIsWhite) (cpBefore - cpAfter).coerceAtLeast(0) else (cpAfter - cpBefore).coerceAtLeast(0)
            ...
            _ui.update { it.copy(lastBadge = MoveReport(uci, badge, loss, exp)) }
            analyzeCurrent()
        }
    }
    ...
    private fun analyzeCurrent() {
        ...
        viewModelScope.launch {
            val (best, cp) = StockfishNative.evaluate(board.fen, 14)
    ...
    fun generateFullReport() {
        viewModelScope.launch {
            ...
            for (uci in _ui.value.history) {
                val fenBefore = tempBoard.fen; val stmIsWhite = tempBoard.sideToMove == Side.WHITE
                val mv = parseUci(uci, tempBoard) ?: continue; tempBoard.doMove(mv)
                val (bestBefore, cpBefore) = StockfishNative.evaluate(fenBefore, 12)
                val (bestAfter, cpAfter) = StockfishNative.evaluate(tempBoard.fen, 12)
                val loss = if (stmIsWhite) (cpBefore - cpAfter).coerceAtLeast(0) else (cpAfter - cpBefore).coerceAtLeast(0)
                val badge = when { loss <= 15 -> Badge.BEST; loss <= 50 -> Badge.GOOD; loss <= 150 -> Badge.INACCURACY; loss <= 300 -> Badge.MISTAKE; else -> Badge.BLUNDER }
    ...
    fun newGame() {
        ...
        _ui.update { UiState(pieces = snapshotPieces(), status = "Stockfish 18 Ready") }; analyzeCurrent()
    }
